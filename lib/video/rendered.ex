defmodule Video.Rendered do
  ...
  @autogen_module_name 'Elixir.Data.AutoGenerated.Video.'
  ...
  """
  @spec get(Video.TrimmedSourceSequence.hash()) :: t() | nil
  ...
  def save_from_tsv_seq(tsv_seq) do
    content = code_from_tsv_seq(tsv_seq)
    path = Path.join(@autogen_file_path, "#{tsv_seq.hash}.ex")
  ...
  def code_from_tsv_seq(tsv_seq) do
    date = Date.utc_today()
    date = :io_lib.format("~4..0B-~2..0B-~2..0B", [date.year, date.month, date.day])
    ...
    stream_path = Video.Path.fully_rendered?(tsv_seq.hash)

    quote do
      defmodule unquote(:"Elixir.Data.AutoGenerated.Video.Rendered_#{tsv_seq.hash}") do
        @moduledoc """
  ...
  @callback created() :: binary()
  @callback hash() :: binary()
  @callback length_ms() :: integer()
  @callback sources() :: Video.TrimmedSourceSequence.sources()
