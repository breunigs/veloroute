defmodule Video.Rendered do
  ...
  import Video.Track, only: [valid_hash: 1, valid_fade: 1]
  ...
  def save_from_track(track) do
    ...
    with content <- as_code(name, hash, track.fade, track.videos, coords),
  ...
  defp as_code(name, hash, fade, sources, coords) when valid_fade(fade) do
    ...
    quote do
      defmodule unquote(module_name(hash)) do
        ...
        @impl Video.Rendered
        def fade(), do: unquote(Macro.escape(fade))
        @impl Video.Rendered
  ...
  @callback fade() :: Video.Track.fade()
