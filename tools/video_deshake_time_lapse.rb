...
# Since time lapsed videos are usually at 29.97 FPS and 5x faster than a regular
...
ARGV.each do |file|
  ...
  Dir.mktmpdir("video_deshake_time_lapse") do |temp_dir|
    ...
    `nice -n18 ffmpeg -hide_banner -loglevel warning -r 29.97 -i "#{file}" -vf "vidstabdetect=stepsize=6:shakiness=8:accuracy=9:result=#{temp_dir}/shake.trf" -f null -`
    binding.pry unless $?.success?

    warn "  fixing speed and smoothing"
    `nice -n18 ffmpeg -hide_banner -loglevel warning -hwaccel auto -r 29.97 -i "#{file}" -vf "vidstabtransform=zoom=1:smoothing=30:input=#{temp_dir}/shake.trf,setpts=1/4*PTS" -an -c:v ffv1 "#{temp_dir}/time_lapse.mkv"`
    binding.pry unless $?.success?
